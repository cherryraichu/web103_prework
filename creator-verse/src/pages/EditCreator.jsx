...
const EditCreator = () => {
    ...
    useEffect(()=>{
        if (nameRef.current) {
            nameRef.current.value = currentForm[0]?.name || "";
        }

        if (descriptionRef.current) {
            descriptionRef.current.value = currentForm[0]?.description || "";
        }

        if (socialMediaRef.current) {
            socialMediaRef.current.value = currentForm[0]?.socialMedia || "";
        }

        if (imgRef.current) {
            imgRef.current.value = currentForm[0]?.image || "";
        }
    },[currentForm.name,
        currentForm.description,
        currentForm.socialMedia,
        currentForm.image
    ])
